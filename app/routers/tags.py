...
async def search_questions_by_topic(query: str, include_thumbnails: bool = Query(True, description="Include thumbnails (slower but better UX)")):
    ...
    # Fetch thumbnails in parallel if requested (much faster)
    if include_thumbnails and matching_items:
        from concurrent.futures import ThreadPoolExecutor
        
        def fetch_thumbnail_data(item):
            video_link = item.get('video_link', '')
            base_url = video_link.split('&t=')[0] if '&t=' in video_link else video_link
            video_id = base_url.split('watch?v=')[1] if 'watch?v=' in base_url else None
            timestamp = _extract_timestamp_from_url(video_link)
        ...
        # Fetch thumbnails in parallel (much faster than sequential)
        with ThreadPoolExecutor(max_workers=10) as executor:
            questions = list(executor.map(fetch_thumbnail_data, matching_items))
    else:
